exports.httpBackendMock = ->
    ...
    .run ($httpBackend, $cookies, utils, API_URL)->
        users = {}
        user_list = []
        sessions = {}

        $httpBackend.whenGET(API_URL + "/users/me").respond (method, url, raw_data) ->
        ...
        $httpBackend.whenPOST(API_URL + "/auth/signup/").respond (method, url, raw_data) ->
        ...
        $httpBackend.whenPOST(API_URL + "/auth/login/").respond (method, url, raw_data) ->
            data = utils.from_form_params(raw_data)
        ...
        $httpBackend.whenGET(API_URL + "/auth/logout/").respond (method, url, raw_data) ->
            if $cookies.sessionId and (user_id = sessions[$cookies.sessionId])
